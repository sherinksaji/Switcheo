...
function formatTokenBalance(balance: ethers.BigNumberish, decimals: number): string {

  const balanceInUnits = ethers.formatUnits(balance, decimals);
  const [integerPart, decimalPart] = balanceInUnits.split('.');
...
async function retrieveHolders() {
  for (const holder of holders) {
    ...
    console.log(`${holder} ${formattedBalance}`);
  }
}
